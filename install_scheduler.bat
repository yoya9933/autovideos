...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
    ...
    "Set-TaskXml $env:EARLY_XML 'Auto publish YouTube Shorts - Morning run (09:00)' '2026-06-06T09:00:00';" ^
...
echo [2/5] Creating morning 09:00 task...
schtasks /Create /TN "VideoTurn\AutoPublish_0900" /XML "%SCRIPT_DIR%task_0900.xml" /F
if errorlevel 1 (
    echo     [FAIL] Please run as Administrator!
    pause
    exit /b 1
)
echo     [OK] 09:00 task created.

echo [3/5] Creating noon 12:00 task...
...
echo [4/5] Creating evening 20:00 task...
...
echo [5/5] Disabling legacy root 09:00 task if present...
schtasks /Query /TN "\MoneyPrinterTurbo Daily Auto Publish" >nul 2>&1
if errorlevel 1 (
    echo     [SKIP] Legacy root task not found.
) else (
    schtasks /Change /TN "\MoneyPrinterTurbo Daily Auto Publish" /DISABLE
    if errorlevel 1 (
        echo     [FAIL] Failed to disable legacy root task.
        pause
        exit /b 1
    )
    echo     [OK] Legacy root task disabled.
)
...
echo   - Daily 12:00 auto upload (video 2)
echo   - Daily 20:00 auto upload (video 3)
echo.
