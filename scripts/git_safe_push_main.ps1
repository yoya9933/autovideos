...
function Assert-SafePaths {
    ...
    $sensitivePatterns = @(
        ...
        '(^|/)[^/]+\.env$',
        '(^|/)[^/]*client_secret[^/]*\.json$',
        '(^|/)(authorized_user|service_account|service-account)\.json$',
        '(^|/)(credentials|token|tokens)\.json$',
        '(^|/)[^/]+_(credentials|token|tokens)\.json$',
        '(^|/)(credentials|token|tokens)_[^/]+\.json$',
        '(^|/)(credentials|token|tokens)\.(pickle|pkl)$',
        '(^|/)[^/]+_(credentials|token|tokens)\.(pickle|pkl)$',
        '(^|/)(credentials|token|tokens)_[^/]+\.(pickle|pkl)$',
        '(^|/)oauth_helper\.',
        '(^|/)storage(/|$)',
        '(^|/)logs(/|$)',
