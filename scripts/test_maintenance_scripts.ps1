...
function Test-SafePushBlocksLocalDatabaseArtifacts {
    Invoke-WithTempRepo {
        param($fixture)
        ...
        Set-Content -Path $baseline -Value "" -Encoding UTF8
        $checkScript = New-PassingCheckScript -Directory $fixture.Root
        Set-Content -Path (Join-Path $fixture.Repo "local_state.db") -Value "local state" -Encoding UTF8
        Set-Content -Path (Join-Path $fixture.Repo "upload_cache.sqlite3") -Value "cache" -Encoding UTF8
        ...
        Assert-True -Condition ($result.ExitCode -ne 0) -Message "Expected local database artifact names to fail."
        Assert-Contains -Text $result.Output -Expected "Refusing to commit sensitive/generated paths" -Message "Expected sensitive path failure message."
        Assert-Contains -Text $result.Output -Expected "local_state.db" -Message "Expected database file to be blocked."
        Assert-Contains -Text $result.Output -Expected "upload_cache.sqlite3" -Message "Expected SQLite file to be blocked."
    }
}
...
$tests = @(
    ...
    "Test-SafePushBlocksLocalDatabaseArtifacts",
