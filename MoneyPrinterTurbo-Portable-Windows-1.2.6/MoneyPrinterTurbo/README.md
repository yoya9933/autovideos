...
如果你想先试最小可用版本，可以直接跑新增的 `auto_publish_youtube.py`：

1. 在 `config.toml` 里配置 `daily_rss_feeds`，再填好 `youtube_upload_enabled`、`youtube_client_id`、`youtube_client_secret`、`youtube_refresh_token`。
...
## Multi-platform publishing: Instagram and Facebook Reels

`auto_publish_youtube.py` now supports a native publisher layer. The default
configuration keeps the existing YouTube-only behavior:

```toml
daily_publish_platforms = ["youtube"]
```

To add Instagram Reels to the same generated short video, use:

```toml
daily_publish_platforms = ["youtube", "instagram_reels"]
instagram_reels_enabled = true
instagram_reels_video_url_template = "https://cdn.example.com/videos/{filename}"
```

To add Facebook Page Reels as the second Meta platform, use:

```toml
daily_publish_platforms = ["youtube", "instagram_reels", "facebook_reels"]
facebook_reels_enabled = true
```

Keep Instagram credentials in environment variables instead of `config.toml`:

```powershell
$env:INSTAGRAM_REELS_ENABLED="true"
$env:INSTAGRAM_IG_USER_ID="<instagram-professional-account-id>"
$env:INSTAGRAM_ACCESS_TOKEN="<instagram-access-token>"
$env:INSTAGRAM_REELS_VIDEO_URL_TEMPLATE="https://cdn.example.com/videos/{filename}"
```

Keep Facebook Page credentials in environment variables too:

```powershell
$env:FACEBOOK_REELS_ENABLED="true"
$env:FACEBOOK_PAGE_ID="<facebook-page-id>"
$env:FACEBOOK_PAGE_ACCESS_TOKEN="<facebook-page-access-token>"
```

Instagram Reels publishing uses the Graph API container flow: create a Reel
media container from a public video URL, poll the container status, then publish
the container. Facebook Reels publishing uses the Page Reels flow: start a Reel
upload session, upload either the local MP4 file or a configured CDN URL, poll
the video status, then publish the Reel. Each run writes platform status to
`storage/auto_publish/publish_status/<task_id>.json`, separate from
`storage/auto_publish/seen.json`.

Useful checks:

```powershell
.\..\lib\python\python.exe auto_publish_youtube.py --dry-run --publish-platforms youtube,instagram_reels,facebook_reels
.\..\lib\python\python.exe auto_publish_youtube.py --no-upload --publish-platforms youtube,instagram_reels,facebook_reels
```

Use the first command to validate RSS selection, prompts, caption input, and the
target platform list before generating video. Use the second command after a
video is generated to confirm job metadata and platform status without uploading.
Actual Instagram publishing also requires the generated MP4 to be available at
the configured public URL. Facebook Reels can upload the generated local MP4
directly. If you prefer CDN upload, set:

```powershell
$env:FACEBOOK_REELS_VIDEO_URL_TEMPLATE="https://cdn.example.com/videos/{filename}"
```

Both Meta publishers support platform-specific caption templates and hashtags
through `instagram_reels_*` and `facebook_reels_*` settings.
