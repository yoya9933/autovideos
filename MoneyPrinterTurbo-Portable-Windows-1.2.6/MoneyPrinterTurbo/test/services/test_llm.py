...
class TestVideoSearchTerms(unittest.TestCase):
    def test_generate_terms_normalizes_english_stock_terms(self):
        with patch.object(
            llm,
            "_generate_response",
            return_value='["AI data center", "台積電晶片", "very long search term phrase"]',
        ):
            terms = llm.generate_terms("AI 晶片新聞", "台積電與資料中心需求升溫", amount=3)

        self.assertEqual(terms, ["ai data center", "very long search"])

    def test_generate_terms_falls_back_to_english_terms_on_llm_error(self):
