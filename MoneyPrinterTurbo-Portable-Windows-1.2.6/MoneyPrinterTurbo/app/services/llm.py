...
def _gemini_finish_reason_name(finish_reason) -> str:
...
def _is_gemini_normal_finish(finish_reason) -> bool:
    return _gemini_finish_reason_name(finish_reason) == "STOP"


def _extract_gemini_candidate_text(candidate) -> str:
    finish_reason = getattr(candidate, "finish_reason", None)
    if not _is_gemini_normal_finish(finish_reason):
        raise ValueError(
            "[gemini] returned non-normal finish reason: "
            f"{_gemini_finish_reason_name(finish_reason) or 'unknown'}"
        )

    content = getattr(candidate, "content", None)
    parts = getattr(content, "parts", None) if content is not None else None
    if not parts:
        raise ValueError("[gemini] returned empty response parts")

    text_parts = [getattr(part, "text", "") for part in parts]
    return _normalize_text_response("".join(text_parts), "gemini")
...
def _get_gemini_api_keys(primary_api_key: str | None) -> list[tuple[str, str]]:
    keys: list[tuple[str, str]] = []
    if primary_api_key:
        keys.append(("primary API", primary_api_key))

    backup_api_key = config.app.get("gemini_backup_api_key", "")
    if backup_api_key and backup_api_key != primary_api_key:
        keys.append(("backup API", backup_api_key))

    return keys


def _has_balanced_script_quotes(text: str) -> bool:
    pairs = [("「", "」"), ("『", "』"), ("“", "”"), ("‘", "’")]
...
def _looks_like_truncated_script(text: str) -> bool:
    value = (text or "").strip()
    if not value:
        return True
    return not _ends_with_sentence_punctuation(value) or not _has_balanced_script_quotes(value)
        ...
        if llm_provider == "g4f":
            ...
            if llm_provider not in ["pollinations", "ollama", "litellm"]:  # Skip validation for providers that don't require API key
                if not api_key and not (
                    llm_provider == "gemini" and _get_gemini_api_keys(api_key)
                ):
                    raise ValueError(
            ...
            if llm_provider == "gemini":
                ...
                generation_config = {
                    ...
                    "top_p": 1,
                    "top_k": 1,
                    "max_output_tokens": int(config.app.get("gemini_max_output_tokens", 8192) or 8192),
                }
                ...
                for key_name, gemini_api_key in _get_gemini_api_keys(api_key):
                    if not base_url:
                    ...
                    try:
                        logger.info(f"requesting gemini with {key_name}")
                        response = model.generate_content(prompt)
                        candidates = response.candidates
                        if not candidates:
                            raise ValueError("[gemini] returned empty candidates")
                        generated_text = _extract_gemini_candidate_text(candidates[0])
                        return _normalize_text_response(generated_text, llm_provider)
                    except (AttributeError, IndexError, ValueError) as e:
                        logger.warning(
                            f"gemini returned invalid response content: {str(e)}"
                        )
                        raise ValueError(str(e) or f"[{llm_provider}] returned invalid response content")
                    except Exception as e:
                        if _is_gemini_quota_error(e):
                            last_quota_error = e
                            logger.warning(
                                f"gemini {key_name} quota/rate limit reached; "
                                "trying backup API if configured"
                            )
                            continue
                        raise
                ...
                raise ValueError("gemini: api_key is not set, please set it in the config.toml file.")
...
def generate_script(
    ...
    for i in range(_max_retries):
        try:
            ...
            if final_script and "Error: " in final_script:
            ...
            if final_script and _looks_like_truncated_script(final_script):
                raise ValueError("generated script looks incomplete or truncated")
...
""".strip()
    ...
    for i in range(_max_retries):
        try:
            ...
            if not isinstance(parsed_terms, list) or not all(isinstance(term, str) for term in parsed_terms):
                logger.error("response is not a list of strings.")
                continue
            search_terms = _normalize_stock_video_terms(parsed_terms, amount)
    ...
    if not search_terms:
        search_terms = _fallback_stock_video_terms(video_subject, video_script, amount)
        logger.warning(f"using fallback stock video terms: {search_terms}")

    logger.success(f"completed: \n{search_terms}")
    return search_terms


def _normalize_stock_video_terms(terms: List[str], amount: int) -> List[str]:
    ...
    for term in terms:
        ...
        if len(normalized_terms) >= amount:
            break
    return normalized_terms


def _fallback_stock_video_terms(video_subject: str, video_script: str, amount: int) -> List[str]:
    text = f"{video_subject} {video_script}".lower()
    terms: list[str] = []
    for keywords, term in STOCK_VIDEO_KEYWORD_MAP:
