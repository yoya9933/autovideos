...
這個工作區是以 `MoneyPrinterTurbo` 可攜版為核心的 Windows 自動短影音流程。它依四條內容線從 RSS 新聞源挑選題材，產生繁體中文 Shorts 旁白與 9:16 影片，並依設定發布到 YouTube Shorts；程式內也已放入 Instagram Reels 與 Facebook Reels 的發布介面。

目前的自動發布目標是每天 24 支：`tech`、`consumer_money`、`cybersecurity`、`science_future` 各 6 支。
...
├─ update_and_run.bat
├─ task_tech_4h.xml
├─ task_consumer_4h.xml
├─ task_cybersecurity_4h.xml
├─ task_science_future_4h.xml
...
.\run_daily_auto_publish.bat --topic-profile cybersecurity
.\run_daily_auto_publish.bat --topic-profile science_future
...
- `--topic-profile tech|consumer_money|cybersecurity|science_future`: 指定內容線；未指定時讀取 `daily_default_topic_profile`，預設為 `tech`。
...
daily_voice_rates = { "zh-TW-HsiaoYuNeural-Female" = 1.40 }
...
[app.daily_topic_profiles.cybersecurity]
candidate_count = 24
focus_keywords = ["資安", "駭客", "漏洞", "勒索軟體", "資料外洩", "個資", "釣魚", "詐騙", "惡意程式"]
editorial_brief = "先說明帳號、金錢、個資或裝置風險；不要提供可濫用的攻擊步驟。"

[app.daily_topic_profiles.science_future]
candidate_count = 24
focus_keywords = ["太空", "能源", "機器人", "新材料", "量子", "科學突破"]
editorial_brief = "先說明突破改變了什麼，並交代證據與限制；不把早期研究說成成熟產品。"
```
...
..\lib\python\python.exe daily_job_report.py --send-email --send-email-on-complete --expected-jobs 24
```
...
- 失敗階段與錯誤摘要。
- 疑似錯誤標題。
- 實際發布的四個主題數量，以及是否各達到 6 支。
- 來源與素材搜尋詞統計。

外層排程每次執行都會產生彙總內容，但只有當台北當日累積到第 24 個 job 時才寄出 Gmail 日報；寄送標記會避免同日重跑造成重複寄信。
...
| 工作 | 時間 | Topic profile |
|---|---:|---|
| `VideoTurn\AutoPublish_Tech_4h` | 00:00 起每 4 小時 | `tech` |
| `VideoTurn\AutoPublish_Consumer_4h` | 01:00 起每 4 小時 | `consumer_money` |
| `VideoTurn\AutoPublish_Cybersecurity_4h` | 02:00 起每 4 小時 | `cybersecurity` |
| `VideoTurn\AutoPublish_ScienceFuture_4h` | 03:00 起每 4 小時 | `science_future` |

排程分配維持四類各 6 支。某個 profile 沒有合格候選時會跳過，不會拿另一條內容線補位；若時段失敗，需要用相同 `--topic-profile` 補跑。
