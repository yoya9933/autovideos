...
這個工作區是以 `MoneyPrinterTurbo` 可攜版為核心的 Windows 自動短影音流程。主要用途是從 RSS 新聞源挑選題材，產生繁體中文 Shorts 旁白與 9:16 影片，並依設定發布到 YouTube Shorts；程式內也已放入 Instagram Reels 與 Facebook Reels 的發布介面。
...
4. 從 `daily_rss_feeds` 抓 RSS/Atom 條目。
5. 用 `daily_rss_state_file` 記錄已處理條目，避免重複發布。
6. 過濾低資訊量與重複事件，收集 `daily_candidate_count` 個候選。
7. 由 LLM 挑選最適合 Shorts 的題材；LLM 失敗時用本機評分退回。
...
- `jobs/*.json`: 每次執行的題材、標題、影片路徑、成功狀態、錯誤階段與警告。
